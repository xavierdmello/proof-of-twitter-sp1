...
async fn prove(req_body: String) -> impl Responder {
    ...
    let request_data: ProveRequestBody =
        serde_json::from_str(&req_body).expect("failed to parse request body");
    ...
    match generate_dkim(email).await {
        Ok(dkim) => {
            ...
            let proof_file = tokio::fs::read("proof-with-io.json")
                .await
            ...
            HttpResponse::build(StatusCode::OK)
                ...
                .append_header((
                    "Content-Disposition",
                    "attachment; filename=\"proof-with-io.json\"",
                ))
                .body(proof_file)
        }
        Err(_) => {
            // Return an error response if generate_dkim failed
            HttpResponse::build(StatusCode::BAD_REQUEST).body("Failed to parse email")
        }
    }
}
...
async fn verify(req_body: web::Bytes) -> impl Responder {
    ...
    let save_proof = web::block(move || {
        fs::write("input_proof_to_be_verified.json", &req_body)
            .expect("failed to write input_proof_to_be_verified.json");
    });
...
fn generate_proof(dkim: &DKIM, eth_address: String) {
    ...
    println!(
        "Associated crypto address (valid if twitter proven): {}",
        verified_address
    );
...
async fn generate_dkim(email: String) -> Result<DKIM, String> {
    ...
    delete_script
        .await
        .expect("failed to delete generate-dkim.js");
